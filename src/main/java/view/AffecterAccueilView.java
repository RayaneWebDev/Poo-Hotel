...
public class AffecterAccueilView extends JPanel {
    ...
    // ✅ Method to get selected Bagagiste
    ...
    public void refreshAssignments(List<Bagagiste> bagagistes, List<Accueil> accueils) {
        model.setRowCount(0);

        for (Accueil accueil : accueils) {
            for (Bagagiste bagagiste : accueil.getBagagistes()) {
                addRow(bagagiste, accueil);
            }
        }

        cbBagagiste.removeAllItems();
        cbAccueil.removeAllItems();

        for (Bagagiste b : bagagistes) {
            cbBagagiste.addItem(b);
        }
